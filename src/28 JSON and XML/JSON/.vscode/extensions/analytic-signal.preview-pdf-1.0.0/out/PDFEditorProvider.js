...
class PDFEditorProvider {
    ...
    getWebviewContent(webview, uri, fill) {
        function resourceUrl(extensionpath, resource) {
		...
		<html dir="ltr" mozdisallowselectionprint>
		  <head>
			...
			<!-- add meta tag for PDF Preview extension -->
			<meta id="analyticsignal.preview-pdf" content="${webview.asWebviewUri(uri).toString()}">

			<!-- add stylesheet resources for PDF Preview extension -->
		    ${stylesheet(resourceUrl(this.context.extensionPath, 'pdf-js/web/viewer.css'))}
			${stylesheet(resourceUrl(this.context.extensionPath, 'extension/viewer.css'))}

			<!-- This snippet is used in production (included from viewer.html) -->
			<link rel="resource" type="application/l10n" href=${resourceUrl(this.context.extensionPath, 'web/locale/locale.properties')}>

			<!-- add script resources for PDF Preview extension -->
			${script(resourceUrl(this.context.extensionPath, 'pdf-js/build/pdf.js'))}
			${script(resourceUrl(this.context.extensionPath, 'pdf-js/build/pdf.worker.js'))}
			${script(resourceUrl(this.context.extensionPath, 'pdf-js/web/viewer.js'))} 
			${script(resourceUrl(this.context.extensionPath, 'extension/viewer.js'))}
		  </head>
    ...
    getWebviewContent2(webview, uri, fill) {
        function resourceUrl(extensionpath, resource) {
            const onDiskPath = vscode.Uri.file(path.join(extensionpath, 'resources', resource));
            return webview.asWebviewUri(onDiskPath);
        }
...
PDFEditorProvider.viewType = 'analyticsignal.preview-pdf';
