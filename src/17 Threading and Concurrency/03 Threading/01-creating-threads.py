...
Recall that threads are used to perform concurrent tasks.  Threads are ultimately created by the operating system 
(kernel), but as far as we are concerned we make a Python call to start a thread; the Python interpreter then 
contacts the kernel.  Python provides a helper class to manage threads.  Rather confusingly, this class is called 
"Thread".  Objects of this class are NOT threads, just helper objects!
...
Realize that when the new threads start, they need to perform a different task (or function) from the main thread.  
This task is specified as a parameter ("target") when the main thread creates the helper objects:
...
After the "start" method has been called, execution of the main thread and the other threads continues in parallel.
Because the operating system may suspend threads at any time, it is not possible to predict which order code will 
execute unless we use special synchronization objects.

In this example the main thread creates 3 other (worker) threads which all execute the "myfunc" function.  Each of 
these threads terminate when they complete this function.  I've added some random timings to emphasize the parallel 
nature of this program.

The "join()" methods are executed by the main thread to waits for each worker thread to complete in turn, ensuring
the main thread finishes last.  When the main thread completes it calls a terminate process function silently.
Without the "join()" calls the program will end abruptly before the worker threads have finished.
'''
