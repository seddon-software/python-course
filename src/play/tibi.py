...
plt.subplots_adjust(bottom=0.35, wspace=0, hspace=0)
...
def update(frame, ax):
    global sliderValue, blits
    if sliderValue > 0:
        ax[sliderValue-1].set_facecolor('yellow')
        ax[sliderValue-1].redraw_in_frame()
        blits.append(ax[sliderValue-1])
    return blits[-4:]
...
ani = FuncAnimation(fig, update, frames=1000, fargs=(axis,), interval=50, blit=True)
# Display graph
plt.show()
